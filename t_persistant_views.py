...
async def restore_registration_view(bot):
    for tournament in Tournament.load_all_tournaments():
        if tournament.reg_msg_id:
            channel = bot.get_channel(tournament.reg_channel)
            if channel:
                try:
...
async def restore_admin_menu_view(bot):
    for tournament in Tournament.load_all_tournaments():
        if tournament.admin_msg_id:
            ...
            channel = bot.get_channel(tournament.admin_msg_channel_id)
            if channel:
                try:
...
async def restore_kick_views(bot):
    for tournament in Tournament.load_all_tournaments():
        channel = bot.get_channel(tournament.participants_channel_id)
...
async def restore_tournament_channel_view(bot):
    for tournament in Tournament.load_all_tournaments():
        channel = bot.get_channel(tournament.tournament_channel_id)
        if channel:
            try:
